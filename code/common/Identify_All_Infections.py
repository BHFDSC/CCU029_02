...
if test:
    display(
        spark.sql(
            f"SELECT * FROM {preamble}_all_admission_infections_from_tests WHERE PERSON_ID_DEID == ''"
        )
    )
...
if test:
    display(
        spark.sql(
            f"SELECT * FROM {collab_database}.{all_covid_admissions_table_name} WHERE PERSON_ID_DEID == ''"
        )
    )
...
if test:
    display(
        spark.sql(
            f"SELECT * FROM {preamble}_all_admission_infections_from_diagnosis WHERE PERSON_ID_DEID == ''"
        )
    )
...
if test:
    display(
        spark.sql(
            f"SELECT * FROM {collab_database}.{all_admission_infection_dates_table_name} WHERE PERSON_ID_DEID == ''"
        )
    )
