...
# Map
cohort_cleaned = (
    cohort.withColumn("ETHNIC_GROUP", mapping_expr_ethnic_group[col("ETHNIC_CLEAN")])
...
display(
    spark.sql(
        f"SELECT INFECTION_VARIANT_PERIOD, COUNT(*) FROM {collab_database}.{output_table_name} WHERE AGE BETWEEN 2 and 11.9999 GROUP BY INFECTION_VARIANT_PERIOD"
    )
)
...
display(
    spark.sql(
        f"SELECT INFECTION_VARIANT_PERIOD, COUNT(*) FROM {collab_database}.{output_table_name} WHERE AGE BETWEEN 12 and 16.9999 GROUP BY INFECTION_VARIANT_PERIOD"
    )
)
...
display(
    spark.sql(
        f"SELECT SUM(PIMS) / COUNT(*) AS PRE_PLUS_ALPHA_IHR FROM {collab_database}.{output_table_name} WHERE INFECTION_DATE < '2021-05-18'"
    )
)
...
display(
    spark.sql(
        f"SELECT SUM(PIMS) / COUNT(*) AS ALPHA_IHR FROM {collab_database}.{output_table_name} WHERE INFECTION_DATE BETWEEN '2020-12-08' AND '2021-05-17'"
    )
)
...
display(
    spark.sql(
        f"SELECT SUM(PIMS) / COUNT(*) AS DELTA_IHR FROM {collab_database}.{output_table_name} WHERE INFECTION_DATE BETWEEN '2021-05-18' AND '2021-12-13'"
    )
)
...
display(
    spark.sql(
        f"SELECT SUM(PIMS) / COUNT(*) AS BA1_IHR FROM {collab_database}.{output_table_name} WHERE INFECTION_DATE BETWEEN '2021-12-14' AND '2022-02-21'"
    )
)
...
display(
    spark.sql(
        f"SELECT SUM(PIMS) / COUNT(*) AS BA2_IHR FROM {collab_database}.{output_table_name} WHERE INFECTION_DATE BETWEEN '2022-02-22' AND '2022-06-06'"
    )
)
...
display(
    spark.sql(
        f"SELECT SUM(PIMS) / COUNT(*) AS BA5_IHR FROM {collab_database}.{output_table_name} WHERE INFECTION_DATE BETWEEN '2022-06-07' AND '2022-11-11'"
    )
)
...
display(
    spark.sql(
        f"SELECT DATEDIFF(ADMIDATE, SORT_ARRAY(ALL_POSITIVE_TESTS)[0]) AS ADMIDATE_MINUS_FIRST_POS_TEST_DATE FROM {collab_database}.{output_table_name} WHERE TYPE_OF_ADMISSION == 'PIMS' AND DATEDIFF(ADMIDATE, SORT_ARRAY(ALL_POSITIVE_TESTS)[0]) IS NOT NULL ORDER BY ADMIDATE_MINUS_FIRST_POS_TEST_DATE"
    )
)
...
if test:
    display(
        spark.sql(
            f"SELECT TYPE_OF_ADMISSION, COUNT(*), SUM(COVID_BY_TEST), (SUM(COVID_BY_TEST) / COUNT(*)) * 100 FROM {collab_database}.{output_table_name} GROUP BY TYPE_OF_ADMISSION"
        )
    )
...
if test:
    display(
        spark.sql(
            f"""SELECT CASE WHEN TYPE_OF_ADMISSION RLIKE 'Type A' THEN 'Type A' WHEN TYPE_OF_ADMISSION RLIKE 'Type B' THEN 'Type B' ELSE TYPE_OF_ADMISSION END, 100 * SUM(CASE WHEN COVID_BY_TEST == 1 THEN 1 ELSE 0 END) / COUNT(*) FROM {collab_database}.{output_table_name} GROUP BY CASE WHEN TYPE_OF_ADMISSION RLIKE 'Type A' THEN 'Type A' WHEN TYPE_OF_ADMISSION RLIKE 'Type B' THEN 'Type B' ELSE TYPE_OF_ADMISSION END"""
        )
    )
...
if test:
    display(
        spark.sql(
            f"SELECT SUM(CASE WHEN ICU == 1 OR TYPE_OF_ADMISSION == 'PIMS' THEN 1 ELSE 0 END) FROM {collab_database}.{output_table_name}"
        )
    )
...
if test:
    uhc_names_1 = ",\n  ".join(
        list(
            dict.fromkeys(
                [
                    f"SUM(CASE WHEN UHC == '{x.replace('PRESENT_CODES_GREEN_BOOK_UHC_', '').replace('PRESENT_CODES_FIVE_YEARS_GREEN_BOOK_UHC_', '')}' THEN 1 ELSE 0 END) AS N_{x.replace('PRESENT_CODES_GREEN_BOOK_UHC_', '').replace('PRESENT_CODES_FIVE_YEARS_GREEN_BOOK_UHC_', '')}"
                    for x in table.schema.names
                    if "PRESENT_CODES_GREEN_BOOK" in x or "PRESENT_CODES_FIVE_YEARS_GREEN_BOOK" in x
                ]
            )
        )
    )
    ...
    uhc_names_2 = "\n  UNION ALL\n  ".join(
        [
            f"SELECT EXPLODE(ARRAY_DISTINCT(SPLIT({x}, ','))) AS ICD10_CODE, '{x.replace('PRESENT_CODES_GREEN_BOOK_UHC_', '').replace('PRESENT_CODES_FIVE_YEARS_GREEN_BOOK_UHC_', '')}' AS UHC\n  FROM {collab_database}.{output_table_name}"
            for x in table.schema.names
            if "PRESENT_CODES_GREEN_BOOK" in x or "PRESENT_CODES_FIVE_YEARS_GREEN_BOOK" in x
        ]
    )
    ...
    separate_UHC_code_counts = spark.sql(
        f"""
SELECT
  {uhc_names_1},
  ICD10_CODE,
...
if test:
    uhc_names_1 = ",\n  ".join(
        list(
            dict.fromkeys(
                [
                    f"SUM(CASE WHEN UHC == '{x.replace('PRESENT_CODES_MD_UHC_', '').replace('PRESENT_CODES_FIVE_YEARS_MD_UHC_', '')}' THEN 1 ELSE 0 END) AS N_{x.replace('PRESENT_CODES_MD_UHC_', '').replace('PRESENT_CODES_FIVE_YEARS_MD_UHC_', '')}"
                    for x in table.schema.names
                    if "PRESENT_CODES_MD" in x or "PRESENT_CODES_FIVE_YEARS_MD" in x
                ]
            )
        )
    )
    ...
    uhc_names_2 = "\n  UNION ALL\n  ".join(
        [
            f"SELECT EXPLODE(ARRAY_DISTINCT(SPLIT({x}, ','))) AS ICD10_CODE, '{x.replace('PRESENT_CODES_MD_UHC_', '').replace('PRESENT_CODES_FIVE_YEARS_MD_UHC_', '')}' AS UHC\n  FROM {collab_database}.{output_table_name}"
            for x in table.schema.names
            if "PRESENT_CODES_MD" in x or "PRESENT_CODES_FIVE_YEARS_MD" in x
        ]
    )
...
if test:
    PIMS_UHC_code_counts = spark.sql(
        f"""
