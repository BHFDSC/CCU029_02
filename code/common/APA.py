...
print(
    f"Subsetting each of the three lookback datasets to just the IDs included in the cohort (`{input_table_name}`)..."
)
...
def generate_lookback_table_statement(hes_type, periods):
    ...
    for i, table in enumerate(tables):
        col_names += [
            col_name
            for col_name in spark.sql(f"SELECT * FROM {table}").schema.names
            if col_name not in ["PERSON_ID_DEID", "LOOKBACK_DATE"]
        ]
        joins.append(
            f"LEFT JOIN {table} {chr(98 + i)}\nON a.PERSON_ID_DEID = {chr(98 + i)}.PERSON_ID_DEID AND a.LOOKBACK_DATE = {chr(98 + i)}.LOOKBACK_DATE\n"
        )
    col_names_formatted = ",\n  ".join(col_names)
    sql_statement = f"""
SELECT
...
def create_concatenated_cols(periods):
    ...
    for period in periods:
        cols.append(
            " + ".join([f"COALESCE({period}_{source}_COUNT, 0)" for source in ["HES_APC", "HES_OP", "GDPPR"]])
            + f" AS {period}_COUNT"
        )
        cols.append(
            " + ".join([f"COALESCE({period}_{source}_COUNT, 0)" for source in ["HES_APC", "HES_OP"]])
            + f" AS {period}_COUNT_NO_GDPPR"
        )
        cols.append(
            "CONCAT_WS(',', "
            + ", ".join([f"NULLIF({period}_{source}_DIAG, '')" for source in ["HES_APC", "HES_OP", "GDPPR"]])
            + f") AS {period}_DIAG"
        )
        cols.append(
            "CONCAT_WS(',', "
            + ", ".join([f"NULLIF({period}_{source}_DIAG, '')" for source in ["HES_APC", "HES_OP"]])
            + f") AS {period}_DIAG_NO_GDPPR"
        )
    return cols
