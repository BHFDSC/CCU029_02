...
extract_archived_on = str(
    spark.sql(f"SELECT MAX(archived_on) FROM {dars_database}.hes_apc_all_years_archive").first()[0]
)
...
try:
    if archived_on == extract_archived_on:
        print(
            f"Config specifies {archived_on} as the archived-on date, this is the latest (per the hes_apc_all_years_archive)."
        )
    else:
...
def create_base_tables():
    gdppr = spark.sql(
        f"""
    SELECT * FROM {dars_database}.gdppr__archive
    ...
    sgss = spark.sql(
        f"""
    SELECT * FROM {dars_database}.sgss__archive
    WHERE archived_on = '{archived_on}'"""
    )
    ...
    deaths = spark.sql(
        f"""
    SELECT * FROM {dars_database}.deaths__archive
    WHERE archived_on = '{archived_on}'
    """
    )
    ...
    for table in tables.items():
        ...
        if not count:
            table_max_archived_on = str(
                spark.sql(f"SELECT MAX(archived_on) FROM {dars_database}.{archives[table[0]]}").first()[0]
            )
            print(
                f"There is no archive in `{archives[table[0]]}` for the selected date (`{archived_on}`), falling back to the archive from `{table_max_archived_on}`"
            )
            table_replacement = spark.sql(
                f"""
