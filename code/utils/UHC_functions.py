...
def create_uhc_definition_statements(name, includes, includes_5_years, uhc_type, no_gdppr, sep):
  ...
  if name == "OBESITY":
    ...
    lookback_list = "CASE WHEN ADMISSION_IN_WINDOW == 1 THEN DIAG_4_CONCAT ELSE NINE_MONTHS_DIAG" + ("_NO_GDPPR" if no_gdppr else "") + " END"
    ...
    case_option_list = []
    if includes:
      case_option_list.append(f"SIZE(ARRAY_INTERSECT(POSSIBLE_CODES_{uhc_type}_{name}, SPLIT(APA_DIAG" + ("_NO_GDPPR" if no_gdppr else "") + ", ','))) > 0")
    if includes_5_years:
      case_option_list.append(f"SIZE(ARRAY_INTERSECT(POSSIBLE_CODES_FIVE_YEARS_{uhc_type}_{name}, SPLIT(FIVE_YEARS_DIAG" + ("_NO_GDPPR" if no_gdppr else "") + ", ','))) > 0")
...
def generate_full_query(df, input_table, uhc_type, no_gdppr, sep=","):
  query_frags = [create_uhc_definition_statements(*row, uhc_type, no_gdppr, sep) for row in zip(df['name'], df['includes'], df['includes_5_years'])]
