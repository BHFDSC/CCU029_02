...
def create_table(table_name: str, database_name="", if_not_exists=True) -> None:
    spark.conf.set("spark.sql.legacy.allowCreatingManagedTableUsingNonemptyLocation", "true")
    spark.sql(
        f"""CREATE TABLE {'IF NOT EXISTS ' if if_not_exists else ''}{database_name}.{table_name} USING DELTA AS
...
def drop_table(table_name: str, database_name="", if_exists=True):
...
def optimise_table(table_name: str, order_by: str):
    spark.sql(f"OPTIMIZE .{table_name}{' ZORDER BY ' + order_by if order_by else ''}")
